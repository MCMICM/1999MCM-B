function [ifintersect, intersection] = isintersect(A,B,C,D)
...
%%% Argument check.
%-------------------------------------------------------------------------------
%http://www.mathworks.com/matlabcentral/fileexchange/27205-fast-line-segment
...
%-------------------------------------------------------------------------------
X1 = repmat(A(:,1),rows,1);
X2 = repmat(B(:,1),rows,1);
Y1 = repmat(A(:,2),rows,1);
Y2 = repmat(B(:,2),rows,1);
...
intersection = [X1 Y1] + [X2_X1.*u_a Y2_Y1.*u_a];
ifintersect = (0<=u_a) & (u_a<=1) & (0<=u_b) & (u_b<=1);